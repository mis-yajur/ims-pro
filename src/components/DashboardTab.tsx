...
import { ArrowDownRight, ArrowUpRight, Award, Box, Database, Sparkles, TrendingUp, RefreshCw, AlertTriangle, X, ShieldAlert } from "lucide-react";
...
export default function DashboardTab({ onSwitchTab, onShowAddTransaction, onRunReport }: DashboardTabProps) {
  ...
  // States for visual Safety Stock alerts
  const [criticalItems, setCriticalItems] = useState<(LatestStockItem & { safetyStock: number })[]>([]);
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      {/* Floating Visual Toast Notification System for Safety Stock Alerts */}
      <div id="safety-stock-toast-overlay" className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
        <AnimatePresence>
          {criticalItems
            ...
            .map((item) => (
              <motion.div
                key={item.sku}
                id={`toast-alert-${item.sku}`}
                initial={{ opacity: 0, y: 50, scale: 0.95 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, x: 100, transition: { duration: 0.2 } }}
                className="pointer-events-auto bg-slate-900 text-white p-4.5 rounded-2xl shadow-2xl border border-slate-800 flex flex-col gap-2.5 relative overflow-hidden group"
              >
                {/* Visual warning accent band */}
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-amber-500 to-rose-600" />
                
                <div className="flex items-start justify-between gap-3 pl-2">
                  <div className="flex gap-2.5">
                    <span className="p-1.5 bg-amber-500/10 text-amber-400 rounded-lg h-fit flex-shrink-0">
                      <AlertTriangle className="w-4.5 h-4.5" />
                    </span>
                    <div>
                      <h4 className="text-[10px] font-bold text-amber-550/80 tracking-wider uppercase font-mono">
                        Safety Stock Deficit • {item.sku}
                      </h4>
                      <h3 className="text-sm font-extrabold text-white mt-1 leading-tight group-hover:text-amber-300 transition-colors">
                        {item.item_name}
                      </h3>
                    </div>
                  </div>
                  <button
                    onClick={() => setDismissedSkus((prev) => [...prev, item.sku])}
                    title="Dismiss alert"
                    className="p-1 hover:bg-slate-800 text-slate-400 rounded-lg transition-colors cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <div className="pl-9 text-xs text-slate-350 space-y-1.5">
                  <p className="leading-relaxed">
                    Current stock level is <strong className="text-rose-400 font-extrabold">{formatNumber(item.quantity)} {item.unit || "pcs"}</strong>, which falls below its factory-calculated Safety Stock buffer of <strong className="text-amber-400 font-extrabold">{formatNumber(item.safetyStock, 1)} {item.unit || "pcs"}</strong>.
                  </p>
                  
                  {/* Action Shortcuts */}
                  <div className="flex items-center gap-2 pt-1.5">
                    <button
                      onClick={() => onSwitchTab("safety-factor")}
                      className="text-[10px] font-semibold text-amber-400 hover:text-amber-300 transition-colors bg-amber-500/10 hover:bg-amber-500/15 px-2.5 py-1 rounded-lg border border-amber-550/20 cursor-pointer"
                    >
                      Inspect Console
                    </button>
                    <button
                      ...
                      className="text-[10px] font-semibold text-slate-300 hover:text-white transition-colors bg-slate-800 hover:bg-slate-700 px-2.5 py-1 rounded-lg border border-slate-700 cursor-pointer"
                    >
                      Receive Stock
                    </button>
                  </div>
          ...
          {/* Consolidated Toast if multiple alerts remain hidden */}
          {(() => {
            const activeCount = criticalItems.filter((item) => !dismissedSkus.includes(item.sku));
            if (activeCount.length > 3) {
              const hiddenCount = activeCount.length - 3;
              return (
                <motion.div
                  id="toast-alert-consolidated"
                  initial={{ opacity: 0, y: 50, scale: 0.95 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, x: 100 }}
                  className="pointer-events-auto bg-slate-950 text-white p-3.5 rounded-xl shadow-2xl border border-slate-800 flex items-center justify-between gap-3 relative overflow-hidden"
                >
                  <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-indigo-500" />
                  <div className="flex items-center gap-2 pl-2">
                    <ShieldAlert className="w-4 h-4 text-indigo-400" />
                    <span className="text-xs font-semibold text-slate-300">
                      Plus {hiddenCount} other items below safety limit
                    </span>
                  </div>
                  <button
                    onClick={() => onSwitchTab("safety-factor")}
                    className="text-[10px] font-bold text-indigo-400 hover:text-indigo-300 transition-colors uppercase bg-indigo-500/15 px-2.5 py-1.5 rounded border border-indigo-550/30 cursor-pointer"
                  >
                    View All
                  </button>
                </motion.div>
